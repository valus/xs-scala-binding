...
class CrossroadsIOLibrarySpecJNA extends WordSpec with MustMatchers with BeforeAndAfter {
  "CrossroadsIOLibrary - JNA" must {
    ...
    "xs_connect" in {
      val context = xs.xs_init
      val (pub, sub) = (xs.xs_socket(context, CrossroadsIO.XS_PUB), xs.xs_socket(context, CrossroadsIO.XS_SUB))
    ...
    "xs_errno" in { 
      xs.xs_init
      xs.xs_errno must equal(CrossroadsIO.ENOENT)
    }
    "xs_(get|set)sockopt" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_SNDHWM, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_SNDHWM, value, lengthRef) must equal(0)
    ...
    "xs_(poll|sendmsg|recvmsg)" in { 
      ...
      xs.xs_setsockopt(sub, CrossroadsIO.XS_SUBSCRIBE, Pointer.NULL, new NativeLong(0))
      ...
      xs.xs_recvmsg(sub, incomingMsg, CrossroadsIO.XS_DONTWAIT) must equal(-1)
      xs.xs_errno must equal(CrossroadsIO.EAGAIN)
      ...
      items(0).socket = sub
      items(0).events = CrossroadsIO.XS_POLLIN
    ...
    "xs_strerror" in { 
      xs.xs_init
      xs.xs_strerror(CrossroadsIO.ETERM) must equal("Context was terminated")
    }
    "xs_term" in { 
      val context = xs.xs_init
    ...
    "socket options: XS_RCVHWM" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_RCVHWM, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_RCVHWM, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_LINGER" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_LINGER, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_LINGER, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_RECONNECT_IVL" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_RECONNECT_IVL, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_RECONNECT_IVL, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_BACKLOG" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_BACKLOG, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_BACKLOG, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_RECONNECT_IVL_MAX" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_RECONNECT_IVL_MAX, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_RECONNECT_IVL_MAX, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_RCVTIMEO" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_RCVTIMEO, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_RCVTIMEO, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_IPV4ONLY" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_IPV4ONLY, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_IPV4ONLY, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_KEEPALIVE" in {
      ...
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_KEEPALIVE, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_KEEPALIVE, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
    "socket options: XS_SURVEY_TIMEOUT" in {
      val context = xs.xs_init
      val socket = xs.xs_socket(context, CrossroadsIO.XS_SURVEYOR)
      val (offset, sizeInBytes, optionValue) = (0, 4, 1)
      val value = new Memory(sizeInBytes) { setInt(offset, optionValue) }
      val (length, lengthRef) = (new NativeLong(sizeInBytes), new LongByReference(sizeInBytes))
      xs.xs_setsockopt(socket, CrossroadsIO.XS_SURVEY_TIMEOUT, value, length) must equal(0)
      xs.xs_getsockopt(socket, CrossroadsIO.XS_SURVEY_TIMEOUT, value, lengthRef) must equal(0)
      value.getInt(offset) must equal(optionValue)
      xs.xs_close(socket)
      xs.xs_term(context)
    }
